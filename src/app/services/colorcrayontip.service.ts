...
export class ColorcrayontipService {
  ...
  getQuizes() {
    ...
    return this.http.get('http://colorcrayontipbackend.ryannewbold.com/server/api/v1/quizes',
  ...
  getQuizesForStudent(userObject) {
    let token = localStorage.getItem('access_token');
    let body = userObject;
    return this.http.get('/server/api/v1/quizes/username/' + body,
      {headers: new HttpHeaders().set('Authorization', 'Bearer ' + token)}
    );
  }
  ...
  getQuiz(id: number) {
    let token = localStorage.getItem('access_token');
    return this.http.get('/server/api/v1/quizes/' + id,
        {headers: new HttpHeaders().set('Authorization', 'Bearer ' + token)}
      );
  }
